...
* [[free()]] exploit: https://www.win.tue.nl/~aeb/linux/hh/hh-11.html#ss11.2
You might be able to overwrite the memory table pointer so that when `free()` runs, it frees up something other than what it originally allocated. When overwriting the size field, we must take care that the computed location of the next chunk lies in allocated memory, to not to cause segfault in [[free()]]. 
...
http://hamsa.cs.northwestern.edu/readings/heap-overflows/ - I cannot make it work ((((
